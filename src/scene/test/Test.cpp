...
Scene::Test::Test(const Application& application) : m_Application{&application},
                                                    shaderProgram{Renderer::Shader::Parser{"../res/shader/Basic.glsl"}},
                                                    texture{
...
void Scene::Test::render(const Renderer::Renderer& renderer) {
    const auto blue = static_cast<float>(m_Application->getFrameCount() % 256) / 255.0f;
